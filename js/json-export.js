...
function generateTestSections(tea) {
    ...
    const geographyAnalysis = geographyResult.data || {};
    const processingAnalysis = processingResult.data?.analysis || {};
    const teaTypeAnalysis = teaTypeResult.data?.analysis || {};
    const flavorAnalysis = flavorResult.data?.flavor?.analysis || {};
    const compoundAnalysis = compoundResult.data?.analysis || {};
    ...
    const recommendedActivities = activityMatcher.matchActivity(
        compoundAnalysis,
        teaTypeAnalysis,
        flavorAnalysis
    );
    
    const recommendedFoods = foodMatcher.matchFood(
        flavorAnalysis,
        processingAnalysis,
        teaTypeAnalysis
    );
    
    const recommendedTimes = timeMatcher.matchTime(
        compoundAnalysis,
        teaTypeAnalysis,
        processingAnalysis
    );
    ...
    // Add to results
    allResults.seasonMatch = { recommended: recommendedSeasons };
    allResults.activityMatch = { recommended: recommendedActivities };
    allResults.foodMatch = { recommended: recommendedFoods };
    allResults.timeMatch = { recommended: recommendedTimes };
...
function createSeasonMatchMarkdown(seasonMatch) {
    if (!seasonMatch || !seasonMatch.recommended || seasonMatch.recommended.length === 0) {
        return '## Season Matching Results\n\nNo recommended seasons available.';
    }
    
    let markdown = `# Season Matching Results\n\n`;
    
    // Recommended seasons
    markdown += `## Recommended Seasons\n\n`;
    markdown += `This tea is best enjoyed during the following seasons:\n\n`;
    
    seasonMatch.recommended.forEach(season => {
        markdown += `- **${formatString(season)}**\n`;
    });
...
/**
 * Create markdown for activity match section
 * @param {Object} activityMatch - The activity matching results with scores
 * @returns {string} Markdown text
 */
function createActivityMatchMarkdown(activityMatch) {
    if (!activityMatch || !activityMatch.recommended || activityMatch.recommended.length === 0) {
        return '## Activity Matching Results\n\nNo recommended activities available.';
    }
    
    let markdown = `# Activity Matching Results\n\n`;
    
    // Recommended activities
    markdown += `## Recommended Activities\n\n`;
    markdown += `This tea pairs well with the following activities:\n\n`;
    
    // Handle both old format (string array) and new format (objects with name and score)
    activityMatch.recommended.forEach(activity => {
        if (typeof activity === 'string') {
            // Handle old format (just strings)
            markdown += `- **${formatString(activity)}**\n`;
        } else if (activity && typeof activity === 'object' && activity.name) {
            // Handle new format (object with name and score)
            const activityDisplay = formatString(activity.name);
            // Using formatScoreWithBar with maxScore=100 for the normalized scores
    ...
    markdown += `These recommendations are based on analyzing multiple factors:\n\n`;
    markdown += `- L-theanine and caffeine balance (stimulation/relaxation levels)\n`;
    markdown += `- Compound profile and focus characteristics\n`;
    markdown += `- Tea type base properties\n`;
    markdown += `- Flavor profile psychological associations\n\n`;
    
    markdown += `The ActivityMatcher algorithm matches the tea's energetic qualities to activities where they would be most beneficial.`;
    
    return markdown;
}

/**
 * Create markdown for food match section
 * @param {Object} foodMatch - The food matching results with scores
 * @returns {string} Markdown text
 */
function createFoodMatchMarkdown(foodMatch) {
    if (!foodMatch || !foodMatch.recommended || foodMatch.recommended.length === 0) {
        return '## Food Pairing Results\n\nNo recommended food pairings available.';
    }
    
    let markdown = `# Food Pairing Results\n\n`;
    
    // Recommended foods
    markdown += `## Recommended Pairings\n\n`;
    markdown += `This tea pairs well with the following foods:\n\n`;
    
    // Handle both old format (string array) and new format (objects with name and score)
    foodMatch.recommended.forEach(food => {
        if (typeof food === 'string') {
            // Handle old format (just strings)
            markdown += `- **${formatString(food)}**\n`;
        } else if (food && typeof food === 'object' && food.name) {
            // Handle new format (object with name and score)
    ...
    markdown += `\n## Food Pairing Logic\n\n`;
    markdown += `These recommendations are based on analyzing multiple factors:\n\n`;
    markdown += `- Flavor profile complementary relationships\n`;
    markdown += `- Mouthfeel and body characteristics\n`;
    markdown += `- Processing method influence on taste\n`;
    markdown += `- Tea type traditional pairings\n\n`;
    
    markdown += `The FoodMatcher algorithm identifies foods that will either complement or contrast with the tea's flavor profile in a pleasing way.`;
...
function createTimeMatchMarkdown(timeMatch) {
    if (!timeMatch || !timeMatch.recommended || timeMatch.recommended.length === 0) {
        return '## Time of Day Results\n\nNo recommended times available.';
    }
    
    let markdown = `# Time of Day Results\n\n`;
    
    // Recommended times
    markdown += `## Best Times for Consumption\n\n`;
    markdown += `This tea is best enjoyed during the following times of day:\n\n`;
    
    // Handle both old format (string array) and new format (objects with name and score)
    timeMatch.recommended.forEach(time => {
        if (typeof time === 'string') {
            // Handle old format (just strings)
            markdown += `- **${formatString(time)}**\n`;
        } else if (time && typeof time === 'object' && time.name) {
            // Handle new format (object with name and score)
            const timeDisplay = formatString(time.name);
            // Using formatScoreWithBar with maxScore=100 for the normalized scores
    ...
    markdown += `\n## Time Matching Logic\n\n`;
    markdown += `These recommendations are based on analyzing multiple factors:\n\n`;
    markdown += `- Stimulation level and duration profile\n`;
    markdown += `- Relaxation properties\n`;
    markdown += `- Tea type traditional consumption patterns\n`;
    markdown += `- Processing method influence on energy effects\n\n`;
    
    markdown += `The TimeMatcher algorithm balances energy effects with traditional wisdom about when certain teas are most beneficial.`;
